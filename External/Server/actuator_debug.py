...
def main():
    ...
    btnDiscoverDevices = Button(frame1, text='Discover Devices', fg='green', height=1, width=14,
                                command=start_discover_devices)
    ...
    chbLogTagValues = Checkbutton(frame2, text='Log Tags Values', variable=checkVarLogTagValues,
                                  command=setBoolDisplayForLogging)
    ...
    popup_menu_save_tags_list.add_command(label='Click \'Get Tags\' button to save the list',
    ...
    btnStart = Button(frame3, text='Start Update', state='normal', bg='lightgrey', fg='blue', height=1, width=10,
                      relief=RAISED, command=start_update)
    btnStart.pack(side=LEFT, padx=5, pady=1)

    # add a button to stop updating tag value
    btnStop = Button(frame3, text='Stop Update', state='disabled', bg='lightgrey', fg='blue', height=1, width=10,
                     relief=RAISED, command=stop_update)
    ...
    tbTag = EntryResizing(frame3, justify=CENTER, textvariable=selectedTag, font='Helvetica 11',
                          width=(int(800 / char_width) - 24), relief=RAISED)
    ...
    char_width = fnt.measure("0")
    tagValue = LabelResizing(frame4, text='~', fg='yellow', bg='black', font='Helvetica 18',
                             width=(int(800 / char_width - 4.5)), wraplength=800, relief=SUNKEN)
            ...
            if commDD.connected:
                ...
                if str(devices) == '[]':
                    ...
                    for device in devices:
                        ...
                        lbDevices.insert(i * 10 + 6, 'Revision: ' + str(device['revision']['major']) + '.' + str(
                        ...
                        lbDevices.insert(i * 10 + 9,
                                         'Status: ' + str(int.from_bytes(device['status'], byteorder='little')))
    ...
    try:
        ...
        try:
            if driverSelection.get() == 'SLCDriver':
                ...
                if not tags is None:
                    for tag in tags:
                        lbTags.insert(currentTagLine.get(),
                                      tag + ' {elements: ' + str(tags[tag]['elements']) + ', length: ' + str(
                                          tags[tag]['length']) + '}')
                ...
                if not tags is None:
                    ...
                    for tag, _def in commGT.tags.items():
                        ...
                        if _def['tag_type'] == 'struct':
                            ...
                            lbTags.insert(currentTagLine.get(),
                                          tag + dimensions + ' (' + structureDataType + ')' + ' (' + str(
                                              structureSize) + ' bytes)')
                            currentTagLine.set(currentTagLine.get() + 1)
    ...
    try:
        ...
        for key in keys:
            ...
            if tag['tag_type'] == 'struct':
                ...
                if tag['array'] > 0:
                    add_Tag(j, '- ' + key + '[' + str(tag['array']) + ']' + ' (' + structureDataType + ')' + ' (' + str(
                        structureSize) + ' bytes)')
                else:
                    add_Tag(j, '- ' + key + ' (' + structureDataType + ')' + ' (offset ' + str(
                        tag['offset']) + ')' + ' (' + str(structureSize) + ' bytes)')
                ...
                if tag['data_type'] == 'BOOL':
                    add_Tag(j, '- ' + key + ' (offset ' + str(tag['offset']) + ')' + ' (bit ' + str(
                        tag['bit']) + ')' + ' (' + tag['data_type'] + ')')
                else:
                    if tag['array'] > 0:
                        if tag['data_type'] == 'DWORD':
                            add_Tag(j, '- ' + key + '[' + str(tag['array'] * 32) + ']' + ' (offset ' + str(
                                tag['offset']) + ')' + ' (' + tag['data_type'] + ')')
                        else:
                            add_Tag(j, '- ' + key + '[' + str(tag['array']) + ']' + ' (offset ' + str(
                                tag['offset']) + ')' + ' (' + tag['data_type'] + ')')
                    else:
                        ...
                        if len(myTag) == 1:
                            if myTag[0].endswith('}') and '{' in myTag[0]:
                                ...
                                if isinstance(results.value, str):
                                    ...
                                    if (checkVarBoolDisplay.get() == 1) and (
                                            results.value == True or results.value == False):
                            ...
                            for tag in results:
                                if isinstance(tag.value, list):
                                    ...
                                    if isinstance(tag.value, str):
                                        ...
                                        if (checkVarBoolDisplay.get() == 1) and (
                                                tag.value == True or tag.value == False):
                        ...
                        if checkVarLogTagValues.get() == 1:
                            ...
                            if headerAdded:
                                with open('tag_values_log.txt', 'a') as log_file:
                                    strValue = str(datetime.datetime.now()).replace(' ', '/') + ', ' + logValues[
                                                                                                       :-2] + '\n'
